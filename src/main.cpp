...
GPSModule gps;
// Shared GPS gate logic lives in position_logic.cpp so tests can link it directly.
#include "position_logic.cpp"
...
void mqttCallback(char *topic, byte *payload, unsigned int length)
{
  if (String(topic) == "device/esp-001/camera_url")
  {
    ...
    if (z != -1 && waitingForCamera[z])
    {
      ...
      Serial.println("[MQTT CB] S3 URL confirmed for [" + camId + "] — crack_id=" + resolvedCrackId + ", severity=" + String(severity) + "%");
                      ...
                      double &outLat, double &outLng)
{
  ...
  double latRad = asin(sin(radians(ancLat)) * cos(d) + cos(radians(ancLat)) * sin(d) * cos(b));
  double lngRad = radians(ancLng) + atan2(sin(b) * sin(d) * cos(radians(ancLat)),
...
void loop()
{
  ...
  for (int z = 0; z < 3; z++)
  {
    ...
    if (pendingTrigger[z] && (now - crackDetectedTime[z] >= OFFSET_DELAY_MS))
    {
      ...
      {
        Serial.println("📤 cam_cmd sent to [" + ZONE_NAMES[z] + "] — crack_id=" + crackId[z] + (presignedUrl.length() > 10 ? " ✅ presigned" : " ⚠️ no presigned URL"));
      }
      else
      {
...
void updateLocalDisplay(bool faultActive, String activeZone, int activeCh)
{
  ...
  {
    lcd.setCursor(0, 0);
    lcd.print("SYSTEM: NOMINAL RUN ");
    lcd.setCursor(0, 1);
    lcd.print("SCANNING ALL RAILS  ");
    lcd.setCursor(0, 2);
    lcd.print("WIFI CONNECTION: OK ");
    lcd.setCursor(0, 3);
    lcd.print("AWS CLOUD LINKED    ");
  }
}